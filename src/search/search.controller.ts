...
export class SearchController {
  ...
  @ApiOperation({
    summary: '음악 태그 검색 (TODO : bpm 검색에서 사용하도록 변경 필요)',
  })
  @ApiQuery({
    name: 'seed',
    description:
      '랜덤 시드 숫자(정수형 값 입력), Seed에 따라 검색 결과가 달라짐.',
    example: '42',
  })
  @ApiQuery({
    name: 'tags',
  ...
  async searchMusicWithTags(
    ...
    const musics: MusicInfo[] = await this.musicService.getMusics(
      ids,
      tagSearchDto.tag_response,
    );
    return musics;
  }
  @Get('beat/tag')
